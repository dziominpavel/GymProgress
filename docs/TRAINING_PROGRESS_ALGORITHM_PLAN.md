...
Чтобы не было путаницы, лучше показывать **не абсолютный "балл силы"**, а относительную оценку:
- `Индекс формы (0..100)` — позиция текущей тренировки в рамках последних N тренировок этого упражнения (например, N=12).
- `Δ к прошлой тренировке` — например `+6.4%` или `-3.1%`.
...
Важно: **100 — не "потолок навсегда"**. Это лишь лучший результат в текущем окне сравнения. Когда окно обновляется, индекс пересчитывается.

Если хочется максимально просто и прозрачно, можно вообще убрать число 0..100 и оставить:
- статус (`лучше/хуже/без изменений`),
- процент изменения к прошлой тренировке,
- краткую причину.
