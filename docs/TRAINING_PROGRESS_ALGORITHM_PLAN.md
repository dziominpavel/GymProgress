...
## 10. Текущая реализация (v2.5) — Hypertrophy Scoring + legacy для силы/выносливости

С марта 2026 алгоритм переработан: **разная логика по целям**, шкала **100 ≈ личный рекорд по всем компонентам**, сравнение с **средним за последние 3 сессии**.

### 10.1. Гипертрофия v2.5 — составной стимул

Для гипертрофии балл складывается из трёх компонентов, а не из одного объёма. Тяжёлая работа в продуктивном диапазоне повторений не занижается только из-за меньшего тоннажа.

#### Продуктивная зона (zone coefficient)
| ExerciseType | Повторы | Коэффициент |
|---|---|---|
| COMPOUND | 5–10 | 1.00 |
| COMPOUND | 11–15 | 0.95 |
| COMPOUND | 3–4 | 0.75 |
...
| ISOLATION | 8–15 | 1.00 |
| ISOLATION | 6–7 | 0.90 |
| ISOLATION | 16–20 | 0.95 |
| ISOLATION | 21–30 | 0.75 |
| ISOLATION | иначе | 0.50 |

#### Компоненты
- **Tension** = `currentWeight / bestWeightHistory`
- **Productive** = `√(currentStimulusUnits / bestStimulusUnits)` (volume saturation)
- **RepQuality** = `average(zoneCoefficient per set)`

где `stimulusUnits = Σ(reps × zoneCoefficient)`.

#### Весовые коэффициенты
| ExerciseType | Tension | Productive | RepQuality |
|---|---|---|---|
...
#### Итоговый балл
```
compositeRaw = wT × tensionScore + wP × productiveScore + wR × repQuality
rawMetric = compositeRaw × 100
score = (rawMetric − fatiguePenalty × 10).coerceIn(0, 1000)
```

100 ≈ все компоненты на уровне личного рекорда. >100 = побил рекорд.

#### Guardrail
Если одновременно: вес ≥ 105% от базы, все подходы в productive zone (coeff ≥ 0.75), totalReps не упал >30%, fatigue penalty не вырос >0.03 — статус **не может быть WORSE**.

Исключение: синглы/дабллы (1–3 reps) не считаются гипертрофийным прогрессом.

### 10.2. Сила и выносливость — legacy
...
### 10.3. Сравнение для прогресса

- **База:** среднее за **последние 3 сессии** (без текущей).
- **Hypertrophy:** прогресс по composite score (rawMetric).
- **Strength / Endurance:** прогресс по сырой метрике.
- **Порог:** ±5% для статуса Лучше/Хуже.

### 10.4. Качество и штрафы

- **Rep Quality (hypertrophy):** zone coefficient (см. таблицу выше) — от 0.50 до 1.00.
- **Rep Quality (strength/endurance):** 1.0 (в targetRange), 0.6 (nearRange), 0.3 (вне).
- **Fatigue Penalty:** по dropRate: 0 / 0.03 / 0.06 / 0.10.

### 10.5. Отображение

- **Балл:** 100 ≈ рекорд, может быть >100, cap 1000.
- **Стимул (hypertrophy):** составной балл из tension + productive + quality.
- **Статус:** ▲ Лучше / → Без изменений / ▼ Хуже.
- **Процент:** +5.7% / −3.1%.
- **Причина:** Напряжение ↑, Продуктивные повторы ↓, Тяжёлая работа в продуктивной зоне и т.д.
