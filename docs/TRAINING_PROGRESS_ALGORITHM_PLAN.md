...
## 10. Текущая реализация (v2) — метрики по целям

С марта 2026 алгоритм переработан: **разная логика по целям**, шкала **100 = личный рекорд**, сравнение с **средним за последние 3 сессии**.

### 10.1. Метрика по цели

| Цель | Метрика | Описание |
|------|---------|----------|
| Гипертрофия | Объём | `volume = weight × totalReps` |
| Сила | E1RM | Формула Epley по лучшему подходу: `weight × (1 + reps/30)`, reps кэпируются на 15 |
| Выносливость | Объём | Аналогично гипертрофии |

### 10.2. Балл: 100 = личный рекорд

```
yourBest = max(метрика в истории БЕЗ текущей сессии), при первой сессии = metric
rawScore = (metric / yourBest) × 100
score = (rawScore − fatiguePenalty × 10).coerceIn(0, 1000)
```

- **100** — личный рекорд (лучшая сессия в истории).
- **Первая сессия** — 100 (единственная = рекорд).
- **Cap 1000** — при 10× рекорда.

### 10.3. Сравнение для прогресса

- **База:** среднее значение метрики за **последние 3 сессии** (без текущей).
- **Прогресс в %:** `(currentMetric − avgLast3) / avgLast3 × 100`
- **Порог:** ±5% для статуса Лучше/Хуже.

### 10.4. Качество и штрафы

- **Rep Quality:** 1.0 (в диапазоне), 0.6 (допустимо), 0.3 (вне диапазона) — по целям из раздела 3.3.
- **Fatigue Penalty:** по dropRate (падение повторений от первого к последнему подходу): 0 / 0.03 / 0.06 / 0.10.

### 10.5. Отображение

- **Балл:** 100 = рекорд, может быть >100 при улучшении, cap 1000.
- **Статус:** ▲ Лучше / → Без изменений / ▼ Хуже.
- **Процент:** +12.5% / −3.1% (по сырой метрике).
- **Причина:** кратко, например «Объём ↑» или «повторы вне 8–12».
