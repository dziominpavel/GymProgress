...
> **Статус:** упрощённая версия реализована (change `membership-expiry-reminder`).
> Хранится только дата окончания (без даты начала), окно напоминаний `[expiry-6 .. expiry]`
> через `WorkManager` с `PeriodicWorkRequest` 24ч, отдельный канал `membership_reminder`
> (`IMPORTANCE_DEFAULT`). Дата не указана → воркер отменён. Дата прошла → молчим.
> Оригинальный план ниже оставлен как возможное расширение (дата начала + баннер + deep-link).
